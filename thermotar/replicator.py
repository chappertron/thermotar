...
class Replicator:
    ...
    def __init__(self,objects,indexes = None,index_names = None, get_methods = 'inheritable'):
        ...
        try:
            ...
            df = pd.concat([member.data for member in objects],keys=indexes,ignore_index=False,join='outer',names=index_names)
        except AttributeError:
            df = pd.concat(objects,keys=indexes,ignore_index=False,join='outer',names = index_names)
    ...
    def updata(self):
        ...
        try:
            # to do add some sort of index naming
            df = pd.concat([member.data for member in self.replicas.values()],keys=self.replicas,ignore_index=False,join='outer',names=self.index_names)
        except AttributeError:
            df = pd.concat(self.replicas.values(),keys=self.replicas,ignore_index=False,join='outer',names = self.index_names)
