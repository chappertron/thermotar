    ...
    **kwargs,
    ...
    if func == quad_w_min:
        ...
        bounds_lower = (
            ...
            -np.inf,
            G_lower,
        )  # if the quadratic, force it to be positive curvature
    else:
        bounds_lower = -np.inf
    ...
    popt, pcov = optimize.curve_fit(
        ...
        ys_arr,
        sigma=sigmas,
        bounds=(bounds_lower, np.inf),
        absolute_sigma=absolute_sigma,
        p0=p0,
    )  # absolute sigma set with Kwargs. TODO Check if this should be set true by default. Should probably...
...
def get_poly_min(fit, xh=None, xl=None):
    ...
    # filter last so that
    crit_points_real = crit_points_real[
        poln.deriv(2)(crit_points_real) > 0
    ]  # NB 2nd derivative is strictly greater than so that inflection points aren't found
...
def choose_temp_range(df, ptp=200, pot_name="phi_tot", temp_name="temp"):
    ...
    Tl = (
        T_min - ptp / 2
    )  # upper and lower limits of an interval ptp wide centred about T_min
    Th = T_min + ptp / 2
...
def find_min(
    ...
    if (
        n != 2 and not use_alt_quad
    ):  # use the special form for a quadratic G(x-x0)^2 +y_0
        ...
        try:
            fit, pcov = alternate_fit(
                y,
                x,
                xl=xl,
                xh=xh,
                sigma=sigma,
                constrain_curvature=True,
                constrain_min=True,
                func=quad_w_min,
            )
        except RuntimeError:
    ...
    if err and n == 2 and sigma is not None:
        ...
        )  ### return error bars, only if error of data points is accurately accounted for
...
def find_phi_min(
    ...
    if temp_centre is None and (temp_range is not None):
        Tl, Th = choose_temp_range(
            chunk.data, ptp=temp_range, pot_name=potential_name, temp_name=temp_name
        )
        if verbose:
        ...
        if verbose:
            print("Fitting to the full range")
    ...
    if verbose:
        print(f"Fitting a {n}-order polynomial between T = {Tl:.3f},{Th:.3f} K.")

    if not err:
        T_min, phi_min, fit = find_min(
            phis, temps, n, sigma=sigma, xl=Tl, xh=Th, use_alt_quad=use_alt_quad
        )
    else:
        T_min, phi_min, fit, err_params = find_min(
            phis,
            temps,
            n,
            sigma=sigma,
            xl=Tl,
            xh=Th,
            use_alt_quad=use_alt_quad,
            err=err,
        )

    if verbose:
        print(f"Minimum found at T = {T_min:.3f} ")
...
def find_x_intercept(
    y, x, offset=0, xmin=None, xmax=None, interp_grid=None, interp_modde="linear"
):
    """
    ...
    raise NotImplementedError(
        "The method of finding from an interpolated intercept is not yet implemented"
    )


def profile_calculating(
    chunk: Potential,
    w=5,
    sigma=3,
    win_type=None,
    ...
    chunk_smoothed = Potential(
        chunk.data.rolling(w, win_type=win_type).mean(std=sigma).copy()
    )
    ...
    chunk_smoothed.data["drho_dT"] = np.gradient(
        chunk_smoothed.density_mass, chunk_smoothed.temp
    )
    # different ways to calculate

    ## Os theta gradient ratio

    chunk_smoothed.data["cos_theta_grad_T"] = (
        chunk_smoothed.cos_theta / chunk_smoothed.temp_grad
    )
    ...
    if direct:
        ...
        chunk_smoothed.data["STP"] = -1 * np.gradient(
            chunk_smoothed.phi_tot, chunk_smoothed.temp
        )
        chunk_smoothed.data["STP_P"] = -1 * np.gradient(
            chunk_smoothed.phi_P_z, chunk_smoothed.temp
        )
        chunk_smoothed.data["STP_Q"] = -1 * np.gradient(
            chunk_smoothed.phi_Q_zz, chunk_smoothed.temp
        )

    ## calculate pressure. Assume z direction
    try:
        chunk_smoothed.data["Press"] = (
            -1 * (chunk_smoothed.loc_stress_3) * chunk_smoothed.density_number
        )
