...
def my_tail(file,N=10):
    ...
    with open(file) as stream:
        ...
        while not (len(fetched_lines) >= N or stream.tell() == 0): # greater than used to ensure that no partial lines are captured eg if N lines requested is part way through the buffer
            ...
            fetched_lines.extend(stream.readlines()) # the very first line could be partial, by selecting only [1:] this ensures it isn't kept 
...
def _tail(file, N=20):
    ...
    with open(file,'rb') as f: # open file as binary
        ...
        while lines_to_go > 0 and block_end_byte > 0:
            if (block_end_byte - BLOCK_SIZE > 0): # if previous block ended on one larger than the blocksize, see -*n iters from end 
            ...
            block_number -= 1
        all_read_text = b''.join(reversed(blocks)) # is reversed, because last block is found first
    desired_bytes = b'\n'.join(all_read_text.splitlines()[-total_lines_wanted:]) # string it all together with just the lines wanted
    desired_text = desired_bytes.decode('UTF-8') ## perhaps can use what ever encoding is used initially or by default??

    output = io.TextIOWrapper(io.BytesIO(desired_bytes),encoding='utf-8')#io.StringIO(desired_text)
