...
def _lammps_parse_chunk(chunk_file, nchunks="auto", header_rows=3, last_only=False):
    ...
    #     # append to indexes if of the length indices
    #     if line.startswith('#'):
...
def lmp_last_chunk(fname, nchunks="auto", header_rows=3, header_line=None):
    ...
    For convnience this parsing gives the output as pandas objects, with indexes as the
...
def parse_numpy_file(fname, header_row=0, comment="#"):
    """
    Load whitespace separated files generated by np.savetxt
    reads the header suing df_utils.commentheader


    TODO Performance is quite bad here. Bot function calls are taking similar amounts of time. \Might be faster to generate the df from a numpy array
    (using load_txt) and figure out alternate way of finding comment_header.
    """

    header = df_utils.comment_header(
        fname, line_no=header_row, comments=comment, delim=" "
    )
