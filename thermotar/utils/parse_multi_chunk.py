...
class LMPChunksParser:
    ...
    def parse_chunks(self, chunk_len="auto", *, verbose=False, **read_csv_kwargs):
        ...
        if chunk_len == "auto" and arr_n_chunks.min() == arr_n_chunks.max():
            if verbose:
                print("Using the single chunk size parser")
            use_single_size = True
            chunk_size = arr_n_chunks[0]
        else:
            if verbose:
                print("Using the multisize chunk size parser")
            use_single_size = False
    ...
    def parse_chunks_np(self):
        ...
        for line_no, info in self.info_rows_dicts.items():
            ...
            values[info] = pd.DataFrame(
                np.loadtxt(self.fname, skiprows=line_no + 1, max_rows=int(info[1])),
    ...
    def set_index_name(self):
        ind_names = tuple(
            df_utils.comment_header(self.fname, line_no=self.header_line - 1)
        )
        # print(ind_names)
