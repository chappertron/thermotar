...
class Potential(th.Chunk):
    ...
    def calculate_potentials(
        self,
        Ps=["P_x", "P_y", "P_z"],
        Qs=["Q_xx", "Q_yy", "Q_zz", "Q_xy", "Q_xz", "Q_yz"],
        charge="charge_dens",
        coords="coord",
        verbose=False,
        pot_corr=True,
        ...
        sub_df["E_tot"] = (
            cumtrapz(sub_df[charge], coordinates, initial=0) / epsilon_0_AA
        )  # V per angstrom
        # correct by subtracting the average.
        if pot_corr and not alt_corr:
            sub_df["E_tot"] -= sub_df["E_tot"].mean()
        if alt_corr:
            aveE = trapz(sub_df["E_tot"], coordinates) / (
        ...
        sub_df[pot_P_names] = (
            pd.DataFrame(cumtrapz(sub_df[Ps], coordinates, initial=0, axis=0))
            / epsilon_0_AA
        )
        if verbose:
        ...
        if pot_corr:
            for col in pot_Q_names:
                sub_df[col] -= (
                    (sub_df[col].iloc[-1])
                    * sub_df[coords]
                    / (sub_df[coords].max() - sub_df[coords].min())
                )  # remove the last potential so it isn't slopey
            sub_df[field_Q_names] = -1 * np.gradient(
                sub_df[pot_Q_names], sub_df[coords], axis=0
            )  # derivative of the corrected potential field
        if pot_corr:
            ...
            sub_df[field_P_names] = -1 * np.gradient(
                sub_df[pot_P_names], sub_df[coords], axis=0
            )
